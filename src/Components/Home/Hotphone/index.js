...
class hotphone extends Component {
  render() {
      return (
        this.props.dataList
        ? <div id="hotphone">
          {this.props.ish3shouw?<h3>{this.props.protitle}<span>&gt;</span>
      </h3>:null}
          <ReactSwipe className="carousel list">
            {
              this.props.dataList.map((item,index)=>{
                if(index<this.props.upperLimit){
                  return(
                    <div key={item.id} className="looplist" onClick={this.handleClick.bind(this,item.id)}>
